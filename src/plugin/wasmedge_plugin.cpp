...
WasmEdge_Result HostGetCurrentObjectId(void*, const WasmEdge_CallingFrameContext*, const WasmEdge_Value*,
                                       WasmEdge_Value* out) {
  out[0] = WasmEdge_ValueGenI64(static_cast<int64_t>(g_host_ctx.object_id));
  return WasmEdge_Result_Success;
}

WasmEdge_Result HostGetComponent(void*, const WasmEdge_CallingFrameContext* frame, const WasmEdge_Value* in,
                                 WasmEdge_Value* out) {
...
WasmEdge_Result HostSetComponent(void*, const WasmEdge_CallingFrameContext* frame, const WasmEdge_Value* in,
                                 WasmEdge_Value* out) {
  auto object_id = static_cast<uint64_t>(WasmEdge_ValueGetI64(in[0]));
  auto type = static_cast<uint32_t>(WasmEdge_ValueGetI32(in[1]));
  auto buf_ptr = static_cast<uint32_t>(WasmEdge_ValueGetI32(in[2]));
...
WasmEdge_ModuleInstanceContext* CreateHostModule() {
  ...
  auto* get_id_type = WasmEdge_FunctionTypeCreate(nullptr, 0, get_id_out.data(), 1);
  auto* get_id_func = WasmEdge_FunctionInstanceCreate(get_id_type, HostGetCurrentObjectId, nullptr, 0);
  ...
  std::array<WasmEdge_ValType, 4> comp_in = {WasmEdge_ValTypeGenI64(), WasmEdge_ValTypeGenI32(),
                                             WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32()};
  std::array<WasmEdge_ValType, 1> comp_out = {WasmEdge_ValTypeGenI32()};

  auto* get_type = WasmEdge_FunctionTypeCreate(comp_in.data(), 4, comp_out.data(), 1);
  auto* get_func = WasmEdge_FunctionInstanceCreate(get_type, HostGetComponent, nullptr, 0);
  ...
  auto* set_type = WasmEdge_FunctionTypeCreate(comp_in.data(), 4, comp_out.data(), 1);
  auto* set_func = WasmEdge_FunctionInstanceCreate(set_type, HostSetComponent, nullptr, 0);
...
bool WasmEdgePlugin::Execute(GameObject& obj, const std::string& action, const std::vector<uint8_t>& input,
  ...
  if ((state_->memory != nullptr) && !input.empty()) {
    if (input.size() > output_offset)
      return false;
    auto res = WasmEdge_MemoryInstanceSetData(state_->memory, input.data(), input_offset, input.size());
    if (!WasmEdge_ResultOK(res))
      return false;
  }

  auto func_name = WasmEdge_StringCreateByCString("execute");
  auto action_hash = static_cast<int64_t>(CityHash64(action.data(), action.size()));
  std::array<WasmEdge_Value, 5> params = {WasmEdge_ValueGenI64(action_hash),
  ...
  WasmEdge_Value ret;
  auto res = WasmEdge_VMExecute(state_->vm, func_name, params.data(), 5, &ret, 1);
  ...
  if (ret_len > 0) {
    ...
    if (state_->memory != nullptr) {
      auto res2 = WasmEdge_MemoryInstanceGetData(state_->memory, output.data(), output_offset, ret_len);
