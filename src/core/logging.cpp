...
const std::size_t max_file_num = 1000;
...
{
public:
    DateRotatingFileSink(const std::string& base_filename, std::size_t max_size, std::size_t max_files)
        : base_filename_(base_filename), max_size_(std::max(max_size, g_min_file_size)), max_files_(std::min(max_files, max_file_num))
    {
...
private:
    ...
    void CheckDateAndRotate()
    {
        ...
        if (new_filename == base_filename_) return;
