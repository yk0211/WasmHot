...
// on a single GameObject. Each actor owns its own plugin instances so that no
// runtime state is shared across actors. The actor does not keep a raw pointer
// to the GameObject; it looks up a temporary view through ObjectRegistry on demand.
